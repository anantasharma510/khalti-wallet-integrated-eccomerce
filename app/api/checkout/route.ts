...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!shippingAddress.fullName || !shippingAddress.address || !shippingAddress.city || !shippingAddress.postalCode) {
      return NextResponse.json({ message: "All shipping address fields are required" }, { status: 400 })
    }
    ...
    // If payment method is cash on delivery, mark order as processing
    if (paymentMethod === "cash_on_delivery") {
      await db.collection("orders").updateOne({ _id: result.insertedId }, { $set: { status: "processing" } })
    }
